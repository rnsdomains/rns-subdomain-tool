(function () {
    ...
    function registerClick(){
        ...
        if (addr != "" && domain != "") {
            ...
            $("#domain").addClass("is-valid was-validated");
            $.LoadingOverlay("show");
            var url = config.api.host + config.api.resources.registerSubdomain;
            domain = domain + config.custom.parentDomain;            
    ...
    function checkDomainStatus(){
        ...
        if (domainName == "") {
            ...
            $.LoadingOverlay("show");
            var url = config.api.host + config.api.resources.subdomainStatus;
            var domainName = domainName + config.custom.parentDomain;
    ...
    function txStatus(txHash, cb){
        var url = config.api.host + config.api.resources.transactionStatus;        
    ...
    function setParentDomainInfo() {        
        $("#parentDomainCheckStatusForm").text(config.custom.parentDomain);  
        $("#parentDomain").text(config.custom.parentDomain); 
        $("#linkLogoCustom").attr("href", config.custom.logoClickUrl); 
        $("#domainName").text(config.custom.title);    
    }
    ...
    function loadConfig() {
        $.getJSON("config.json", function (json) {
            config = json;
            setParentDomainInfo();
            var dataLocal = window.localStorage.getItem('registrationInProgress');
            if(dataLocal)
                loadInProgressTxScreen(JSON.parse(dataLocal));
        });
    }
    ...
    function showTxProgress(txHash){
        ...
        var href = config.explorer.tx + txHash;
